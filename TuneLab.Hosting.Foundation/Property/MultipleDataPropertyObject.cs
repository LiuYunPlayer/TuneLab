...
public class MultipleDataPropertyObject : IDataPropertyObject
{
    ...
    // 多选下数组按 index 对齐合并：单成员＝等价单选、直通真实数组（保 token 稳定 + seed 越界绑定）；
    // 0/多成员复合成 MultipleDataPropertyArray——长度取最长、逐 index 三态、写扇出到拥有该位的成员。
    public IDataPropertyArray Array(string key)
    {
        if (mDataObjects.Count == 1)
            return mDataObjects[0].Array(key);

        var members = new List<IDataPropertyArray>(mDataObjects.Count);
        foreach (var dataObject in mDataObjects)
            members.Add(dataObject.Array(key));
