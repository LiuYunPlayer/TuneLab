...
public static class IModifiedEventExtensions
{
    ...
    sealed class Merged(IEnumerable<IModifiedEvent> sources) : IModifiedEvent
    {
        public void Subscribe(Action invokable) { foreach (var source in sources) source.Subscribe(invokable); }
        public void Unsubscribe(Action invokable) { foreach (var source in sources) source.Unsubscribe(invokable); }
        public IActionEvent<bool> AsEverytime() => sources.Select(source => source.AsEverytime()).Merge();
    }
}
