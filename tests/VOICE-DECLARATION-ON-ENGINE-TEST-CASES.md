...
> `GetPartPropertyConfig` / `GetNotePropertyConfig`）从 `ISynthesisSession` 上移到 **`IVoiceEngine`**，`voiceId` 经
> **`IPartPropertyContext.VoiceId`** 注入；会话只余 `DefaultLyric`。宿主据此在**建会话之前**填好声明，使会话
