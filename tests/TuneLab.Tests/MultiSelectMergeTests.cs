...
public class MultiSelectMergeTests
{
    ...
    // ===== config 层：PropertyObjectExtensions.Merge =====

    [Fact]
    public void Merge_Scalar_SameValueOrMultiple()
    {
        var merged = PropertyObjectExtensions.Merge([
            Obj(("a", 1.0), ("b", 2.0)),
            Obj(("a", 1.0), ("b", 3.0)),
        ]);
    ...
    public void Merge_SingleOrEmpty_PassesThrough()
    {
        ...
        Assert.Equal(single, PropertyObjectExtensions.Merge([single]));
        Assert.Equal(PropertyObject.Empty, PropertyObjectExtensions.Merge([]));
    }
