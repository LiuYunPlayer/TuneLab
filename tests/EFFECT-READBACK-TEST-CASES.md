...
>    分段形 `DefaultValue=NaN`），曲线数据经 `IEffectSynthesisSession.SynthesizedParameters` 按同一批 key 承载、由宿主
...
- 涉及的 UI 显示名：
  ...
  - **效果器**（链上添加，按 **manifest 声明的显示名**列出）：`Gain`、`Reverse`
    （对应 type id `TLTestGain` / `TLTestReverse`——那是工程文件里存的值，UI 上一律显示名）。
  - **effect 回显轨**：`Gain` 产 **Loudness**（蓝 `#00B0FF`，0..2），由引擎**恒声明**；`Reverse` **无回显**。
  - 显隐按钮都在**参数区标题栏右侧**（小眼睛 + 文本），voice 与各 effect **按源分组**：每组前有一个淡色**源标签**
    （`Voice` / 各 effect 的**显示名**），组间留更大间距。
- 参照实现的回显数据：`Gain` 每处理一段就为该段产一条 **Loudness** 包络回显（对输出按 ~20ms 窗算 RMS），
...
- 选一条 part（任意声库），底部/侧栏给它的 effect 链**添加 `Gain`**。
- 看参数区标题栏右侧 → 出现一个新分组：淡色源标签 **Gain** + 一个 **Loudness** 按钮（小眼睛 + 文本，蓝）。
  若该声库也有 voice 回显（如 Alice 的 Energy），则两组并列：`Voice 👁Energy` ⋯⋯ `Gain 👁Loudness`，组间留更大间距。
...
### 4. 改 gain 实时反映在回显
- 点亮 Loudness 并合成出曲线。把 `Gain` 的 **gain** 滑块调大 → 重新处理后 Loudness 面积**整体抬高**
  （输出更响、RMS 更大）；调小 → 压低。验证回显确由处理产出、随参数走。

### 5. Reverse 无回显
- 给链再加 `Reverse`（或单独加）。标题栏**不出现** `Reverse` 分组、无 Loudness 之外的按钮
...
### 6. 多个 effect 各自成组
- 链上放两个 `Gain`（Gain → Reverse → Gain 之类）。标题栏出现**两个** `Gain` 分组，各带自己的
  **Loudness** 按钮，互不串色/串显隐：点亮第一个不影响第二个。
- 分别点亮 → 两条蓝色面积分别对应**各自那一级**处理后的响度（后级在前级之后，量级可不同）。

### 7. 增删 / 重排即时刷新
- 删除一个 `Gain` → 其分组**立即从标题栏消失**（不必把鼠标移上标题栏）。重排链顺序 → 分组顺序随之更新。
...
### 9. 多源同屏叠加
- 同时点亮 Alice 的 **Energy**（粉）与 `Gain` 的 **Loudness**（蓝），并画一条可编辑轨（如 voice 的 Growl 橙线）。
...
- 既有可编辑 effect 自动化轨（如 `Gain` 的 **Gain Env**、分段 **Formant**）：参数栏绘制**与改动前一致**
