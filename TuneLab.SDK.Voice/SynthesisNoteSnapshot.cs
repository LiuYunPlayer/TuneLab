...
// note 的不可变快照值（worker 线程、将来 worker 进程只读）：边界为全局 tick（乐谱量
// 只给真值域，秒经 snapshot.Timing 换算）、Properties 值拷。触底到值类型：不含任何
// live 数据对象引用（Phonemes 也须是物化时新建的副本），自包含可序列化值树，为跨进程演进留路。
