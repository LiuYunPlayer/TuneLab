...
// 宿主物化的不可变合成快照（IInstrumentContext.GetSnapshot 的返回体）——instrument 专属面。
// 插件在 SynthesizeNext 的同步前缀（数据线程）主动拉取，之后才 offload——worker 永不碰活对象、只读它。
// 形状与 IInstrumentContext 活视图镜像对称，但为纯数据体：无事件、无活引用；构造形态 = 无参 + required init。
//
...
public sealed class InstrumentSnapshot
{
    ...
    public required IReadOnlyList<InstrumentNoteSnapshot> Notes { get; init; }
