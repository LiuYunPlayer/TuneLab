...
// 宿主只推变更流（经 IVoiceSynthesisContext）、驱动调度、读产物展示。
//
...
//（IsContinuation 的刻意无默认是"强制显式表态"的例外，见其注释——不适用本约定。）
public interface IVoiceSynthesisSession : IDisposable
{
    // —— 运行时取值（会话创建后才被消费；声明类 config 已上移到 IVoiceSynthesisEngine，不在此重复）——
    ...
    // 照单消费（显示布局 / 编辑手势同源）。上下文经 note.Next / Previous 自行导航
    ...
    // GetNextPendingSynthesisRange 自报的 SynthesisRange 原样回灌：插件按同一窗口确定性重导出 notelist
    ...
    // （取消不抛 OperationCanceledException：取消是正常调度结局），错误经 Status 看、
    // 是否还有待合成经 GetNextPendingSynthesisRange 看。取消是尽力请求：不可中止的插件把这块跑完才返回，
    // 槽位在 await 真正返回时才释放、不在请求取消时——资源始终封顶在并发上限内。
    // 进度不在此传入——经 Status 状态带（SynthesisStatusSegment.Progress）+ StatusChanged 上报；
    // 将来如需独立推送通道再加性补成员。
    ...
    //
    // **键在与否即语义**（宿主按此分三态显示，请照此报告）：
    //   · 键不在                → "还没有答案"：该块脏 / 合成中。宿主留白，且**相接的邻居也一并留白**——邻居的
    //                             布局边界依赖本 note 的音素几何，未知就不能先铺、免得数据到达时跳变。
    //   · 键在、音节零音素      → "答案是没有音素"：已算完，本 note 就是不发音 / 不归自己发音（如把发音全挂到
    //                             链头的延续 note）。宿主本 note 留白，但**邻居照常显示**——边界已确定。
    //   · 键在、音节有音素      → 常态。
    // 两个空态语义不同、不可互换：拿"零音素"当"还没算完"用，会让相邻 note 跟着一起消失。
