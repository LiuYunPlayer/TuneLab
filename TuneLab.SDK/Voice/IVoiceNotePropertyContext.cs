...
// 不继承 IVoicePartPropertyContext——选中的 note 必同属**一个** part，故 part 维度是单数（单个 PropertyObject），
// 与 part 面板可多选 part（IVoicePartPropertyContext.PartProperties 为列表）的语义不同，强行继承会被迫拿到列表形 part 值。
//
// note config 可同时依赖 part 设置（单 part）与各选中 note 的值；part 改 commit 沿链触发 note 重算。
// 冻结接口纯加性扩展。
public interface IVoiceNotePropertyContext
{
    // 选定声库（= 宿主 Voice.ID）：同 IVoicePartPropertyContext.VoiceId，引擎据此知道求值的是哪个模型。
    string VoiceId { get; }
