...
// 一条「effect 实例 × 一个上游音频段」的持久厚处理器：持有自己那一段的上下文（IEffectSynthesisContext）。
// 段间彼此无共享上下文——各段分别处理后由宿主按时间混音。
//
// 失效判定权归宿主：宿主按作用域信号（本段输入重 Commit / 本 effect 参数变 / 本 effect 自动化变更区间
// 与本段相交）保守调度 Process，处理器不承担任何失效上报义务（编辑批量/收口/防抖全在宿主，处理器不可见）。
// context 的颗粒事件（Input.RangeModified / Properties.Modified / automation.RangeModified）只是缓存型引擎
// 「该重算哪块」的可选信息源，简单引擎可以零订阅。
//
...
public interface IEffectSynthesisProcessor : IDisposable
{
    ...
    // 同步前缀读到的就是此刻最新真相，期间发生过几次编辑、以何种顺序，与处理器无关也不可见。
    ...
    // 本处理器的状态声称时间线（与 voice 会话 GetStatus 同一词汇；范围 = 全局秒、主语 = 本处理器
