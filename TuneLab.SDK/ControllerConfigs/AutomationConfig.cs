...
public sealed class AutomationConfig : IValueConfig<double>
{
    ...
    // 引擎须按自身需求校验。宿主的手绘落笔本身不越界（拖拽/锚点输入钳在量程内），但越界值仍有几个正当来源：
    // 值模型是加性的（锚点存相对 DefaultValue 的偏移、vibrato 偏移叠加到轨上，故用户改默认值或让 vibrato
    // 影响该轨后合成结果会出界），以及把模型回显烘焙进同 Id 可编辑轨时按真实数值原样写入、不按本量程钳位
    // （钳位会静默改数据；回显轨与可编辑轨各按自己量程绘制、宿主不要求二者一致，量程声明不一致的后果即出界）。
