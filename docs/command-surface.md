...
- **带参动作不可绑**。一条绑定只有手势、没有参数，v1 刻意不做"绑定携带参数"（keybinding-system.md §10
  把它留给 v2），故这批动作只从命令面与界面入口触发，`list_actions` 里 `bindable` 恒 false。
...
而且视口仍然不改变任何结果（§11 的理由没变）。选择器参数收的是"界面上本来就摆着一排、成员随工程变"
的那类入口，不是任何缺参数的东西的通用入口。
...
- **另存为 / 打开**：不禁止，但**动作面**上的那两条弹的是要人应答的模态，故 `action run` 默认拒绝、
  ...
  **导出音频**仍是人在环决定（`project export` 只导工程文件）。
- **视口（缩放 / 滚动 / 视图定位）**：曾被列为动作面二期的"明显的洞"，**否决**（issue #150）。滚轮缩放是
  以鼠标位置为轴心的连续手势（操作态，见 keybinding-system.md §0），而外部没有鼠标；更要紧的是**视口不
  改变任何结果**，只改变用户此刻在看什么。真正有用的那件事是"让用户看到我改了哪里"，它要一个 tick 或一个
  对象作参数。**选择器参数落地之后这一条不变**（§5.6）：那收的是"界面上本来就摆着一排、成员随工程变"
  的入口（一个闭集里的成员），而"滚到某个 tick"要的是一个连续量，且仍然不改变任何结果。
