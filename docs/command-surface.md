...
- **闭集逐值收，动态集等参数**。成员固定且有限的（5 支笔、6 个侧栏面、18 档量化）逐值开一条终态动作，
  id 由事实算出而不是抄一张并行的表（`quantization.1_12` = 三连基数 3 × 细分 4）。成员随工程变的
  （参数面板的合成参数轨、参数栏钉选项、最近文件、导出格式族）**不**逐成员开 id——那要的是**选择器参数**，
  等 `run_action` 带参数（快捷键 v1 同样把"绑定携带参数"推到了 v2，见 keybinding-system.md §10）。
