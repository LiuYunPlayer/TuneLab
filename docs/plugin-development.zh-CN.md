...
- **`IVoiceSynthesisPhonemeView`**（声明面读音素当前值）：`string Symbol` / `double Duration` / `double StretchWeight` / `bool IsLead` / `PropertyObject Properties`（该音素属性当前值快照）。可据这些当前值 + 位置进一步条件化 schema。
...
public sealed class VoiceSynthesisNoteSnapshot   // 触底到值类型、无任何活引用
{
    ...
    IReadOnlyList<VoiceSynthesisPhonemeSnapshot> Phonemes { get; }   // 钉死音素冻结表项：每项 { string Symbol; double Duration; double StretchWeight; bool IsLead; PropertyObject Properties }（几何平铺），见 §5.7
...
音素描述符**方向无关**——读入（用户钉死约束）与输出（合成产物）都用同一个 `SynthesizedPhoneme`：只报「标称时长 + 权重 + IsLead」，**不报绝对位置**。定位 / 跨 note 去重叠压缩 / melisma 铺设全由**宿主**按同一时长模型派生（引擎报已压缩的绝对位置会让宿主布局误判相接判据，故只报自然时长、宿主独占布局）。

```csharp
public struct SynthesizedPhoneme { public string Symbol; public double Duration; public double StretchWeight; public bool IsLead; }
```
