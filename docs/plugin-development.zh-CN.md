...
note 之外，引擎还可给**音素**声明用户可编辑的自定义属性——与 `GetNotePropertyConfig` 平行，但声明面与读值面都下沉到单个音素。典型用例：给某个元音挂「咬字强度」滑杆、给辅音挂一组开关。
...
// 每个 IVoiceSynthesisNoteView 现带 Phonemes（该 note 的有序音素）。返回与「选中各 note 的音素
// **扁平展开**」（顺序 = context.Notes 顺序 × 各 note 的 Phonemes 顺序）【索引对齐】的 config 列表，
// list[k] = 第 k 个扁平音素的 schema。
public IReadOnlyList<ObjectConfig> GetPhonemePropertyConfigs(IVoiceSynthesisNotePropertyContext context)
{
    // 不声明任何音素属性的引擎直接返回空列表。
    return context.Notes
        .SelectMany(note => note.Phonemes.Select((ph, i) => ConfigForPhonemeAt(note, note.Phonemes, i)))  // 可依位置 / 邻居 / note 条件化
        .ToList();
}
```

- **required、不是默认接口方法**：`GetPhonemePropertyConfigs` 与 `GetNotePropertyConfig` / `GetPartPropertyConfig` 一样**必须实现**。不声明音素属性的引擎直接 `return Array.Empty<ObjectConfig>();`（或返回空列表）即可。
...
- **返回扁平展开、索引对齐**：返回与「选中各 note 的音素**扁平展开**」**索引对齐**的 config 列表——扁平顺序 = `context.Notes` 顺序 × 各 note 的 `Phonemes` 顺序，`list[k]` 就是第 k 个扁平音素的 schema。**返回空列表 = 所有音素均无属性**；否则列表长度**必须等于扁平音素总数**。一次调用即拿全部选中 note 的音素 schema，**天然支持多选 note**，不必逐音素或逐 note 建 context。
- **schema 可依位置条件化**：因为成批拿到整串音素，schema 可依**音素在 note 内的位置（= 该 note `Phonemes` 索引）/ 邻居 / note 信息**给不同控件——例如首辅音 vs 核（元音）vs 尾辅音返回不同配置，或据 note 音高/歌词调整。
- **`IVoiceSynthesisNoteView.Phonemes`**：该 note 的有序音素（前置辅音 → 核 → 后辅音；音素在 note 内的位置 = 此列表索引），元素为 `IVoiceSynthesisPhonemeView`。
- **`IVoiceSynthesisPhonemeView`**（声明面读音素当前值）：`string Symbol` / `double Duration` / `double StretchWeight` / `PropertyObject Properties`（该音素属性当前值快照）。引导/主体归属由 note 视图的 `LeadingPhonemes` / `BodyPhonemes` 列表成员（+ `double BodyOffset`）给、不落每音素。可据这些当前值 + 位置进一步条件化 schema。
- schema 仍用同一套控件配置词汇搭（`SliderConfig` / `ComboBoxConfig` / `CheckBoxConfig` / `TextBoxConfig`，容器 `ObjectConfig`），与 note 属性写法一致。

```csharp
// 示例：核（元音，StretchWeight > 0）给「咬字强度」，辅音不挂属性（返回空 ObjectConfig）。
...
```csharp
foreach (var ph in note.Phonemes)             // note 为 VoiceSynthesisNoteSnapshot
{
...
> ⚠️ **契约变更**：以前 `snapshot note.Phonemes` 的元素是裸 `SynthesizedPhoneme`，现在是 `VoiceSynthesisPhonemeSnapshot`（几何字段平铺 + `Properties`）。已有代码遍历快照音素时，几何字段直接读 `ph.Xxx`（平铺，如 `ph.Symbol`）。

**语义要点：**

- 属性只在**钉死音素**上有意义（用户数据）；引擎自动 G2P 产出的音素无属性。
- 输入活视图 `IVoiceSynthesisNote.Phonemes` 仍是 `IReadOnlyList<SynthesizedPhoneme>`（**不带属性**，见 §5.3 / §5.7）；属性只出现在合成快照（`VoiceSynthesisPhonemeSnapshot.Properties`）里。
- **编辑 UI**：侧栏音素属性面板已落地——**逐音素一行**（符号标签 + 该音素自己的控制器），按选中 note 成批调 `GetPhonemePropertyConfigs` 求 config。仍待做的是音素的**选中模型**（当前音素无 `ISelectable` 选中态）；在选中模型补齐前，宿主以选中 note 的全体音素为面板范围。
...
| `IVoiceSynthesisEngine.GetPhonemePropertyConfigs` | 数据线程 | per-phoneme 属性面板（required；收 `IVoiceSynthesisNotePropertyContext`，返回与选中各 note 音素扁平展开索引对齐的 config 列表，空列表=无属性） |
...
class MyEffectProcessor : IEffectSynthesisSession
{
    ...
    //（worker 就地报），宿主 marshal 后拉 GetStatus；引擎宜自持粒度（如每整数百分比一报）。
...
- **状态声称（可选）**：经 `GetStatus()` 发布状态时间线（不可变列表换引用）+ 任意线程触发 `StatusChanged`——Synthesizing 段带 `Progress` 在状态带呈纵向水位；Synthesized 段是「声称完成」（软色、非最终，最终绿只来自链尾音频事实）。不报的引擎由宿主按调度事实兜底呈现。
