...
- **`IVoiceSynthesisPhonemeView`**（声明面读音素当前值）：`string Symbol` / `double Duration` / `double StretchWeight` / `PropertyObject Properties`（该音素属性当前值快照）。前后归属由 note 视图的 `double Preutterance` 派生、不落每音素。可据这些当前值 + 位置进一步条件化 schema。
...
- **合成时读值**：从快照读——`VoiceSynthesisNoteSnapshot.Phonemes` 现在是 `IReadOnlyList<VoiceSynthesisPhonemeSnapshot>`，每项 `{ string Symbol; double Duration; double StretchWeight; bool IsLead; PropertyObject Properties }`。几何字段（`Symbol`/`Duration`/`StretchWeight`/`IsLead`）平铺直读；要喂 `PhonemeLayout.Resolve`（吃 `SynthesizedPhoneme`）时按字段重建一个 `SynthesizedPhoneme`（见 §5.7）。`Properties` 是该音素属性的冻结值（未设 = `PropertyObject.Empty`），用 `GetDouble(key, default)` 等读，稀疏存储、读不到回退声明默认值。
...
public sealed class VoiceSynthesisNoteSnapshot   // 触底到值类型、无任何活引用
{
    ...
    double Preutterance { get; }   // 该 note 的拍前发声量（自然秒）；前后归属由它派生
    IReadOnlyList<VoiceSynthesisPhonemeSnapshot> Phonemes { get; }   // 钉死音素冻结表项：每项 { string Symbol; double Duration; double StretchWeight; PropertyObject Properties }（几何平铺），见 §5.7
...
音素描述符**方向无关**——读入（用户钉死约束）与输出（合成产物）都用同一个 `SynthesizedPhoneme`：只报「标称时长 + 权重」，**不报绝对位置、不报前后归属**。前后归属（拍前 / 拍后）由 **note 级 `Preutterance`**（拍前发声量，自然秒）派生、不落每音素标志（消除「交替 IsLead」这类可表示非法态）。定位 / 跨 note 去重叠压缩 / melisma 铺设全由**宿主**按同一时长模型 + Preutterance 派生（引擎报已压缩的绝对位置会让宿主布局误判相接判据，故只报自然时长、宿主独占布局）。

```csharp
public struct SynthesizedPhoneme { public string Symbol; public double Duration; public double StretchWeight; }
// 合成产物 map 值型（IVoiceSynthesisSession.SynthesizedPhonemes 的值）：一组归属 note 的音素 + 前置量。
public readonly struct SynthesizedSyllable { public IReadOnlyList<SynthesizedPhoneme> Phonemes; public double Preutterance; }
```

**输入（host → engine）：`note.Phonemes`（`IReadOnlyList<SynthesizedPhoneme>`，per note）**

- 用「时长 + 权重」（+ note 的 `Preutterance`）表示，而非解析后的绝对时间——相邻钉死 note 的去重叠（后盖前 / **跨 note 辅音簇压缩**）须由**全局布局算法**联合求解；且用时长便于「推挤式」编辑（改一个音素长度，相邻整体平移而非互相挤占）。
  ...
  - **`Preutterance`（per note，自然秒）**：note 头之前音素的占位长度（拍前 / lead 发声量）= note 头落在音素自然时间带上的偏移（从首音素起点量起）。前后归属由它**派生**：落在 Preutterance 之前的音素为前置（拍前），被 note 头切中的音素**跨拍**（前半归前 note 压缩域、后半归本域）。`Preutterance = 0` ⇒ 元音起手（无前置）；要把 N 个前置辅音卡在边界，令 Preutterance = 其时长之和。
  - **位置不存、由布局派生**：note 头是压缩域分界线；前置料从它往左累积（可越界到 note 前）、核 + 后辅音往右——辅音用固定时长、核填充到 **note 满末**（含 melisma 铺过乘客）、多核按权重分摊。
...
**钉死覆盖**：`snapshot.Notes[i].Phonemes` 非空 = 该 note 用户钉死，物化 `PhonemeLayoutNote.Phonemes` 时用其钉死音素而非你的 G2P 预测；空才用预测。注意快照的 `Phonemes` 元素现在是 `VoiceSynthesisPhonemeSnapshot`（不再是裸 `SynthesizedPhoneme`），喂布局的几何按 `ph` 的字段重建 `SynthesizedPhoneme`（`{ Symbol = ph.Symbol, Duration = ph.Duration, StretchWeight = ph.StretchWeight, IsLead = ph.IsLead }`），per-phoneme 属性取 `ph.Properties`（见 §5.2「音素属性」与 §5.7 末）。
...
- **核时长是基准比例、单核时被抵消**：核（`StretchWeight>0`）的 `Duration` 是其原长——**多核**时定彼此的基准比例（各乘 `r^w`）；**单核**时原长被抵消、退化为填满核空间，报多少都一样。辅音（w=0）的 `Duration` 即其固定长。`IsLead` 决定前后归属（前置往左累积、核 + 后辅音往右）。因此输出**无需自己摆位**，只诚实报每个音素的时长 + 权重 + 前后标记即可，锁定时宿主按「核起点 = 音符头」统一派生位置——零跳变。
