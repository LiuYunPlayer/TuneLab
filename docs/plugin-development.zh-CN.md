...
实现 `IEffectSynthesisEngine`。需要**无参构造函数**。效果器 id 写在 `manifest.json` 的 `engine`，实现类列进 `classes`（宿主按 `IEffectSynthesisEngine` 接口认领，不再用 attribute）。引擎是每种效果器类型一个；宿主为工程里每条「effect 实例 × 上游音频段」创建一个**持久厚处理器** `IEffectSynthesisProcessor` 驱动它。处理器持有自己那一段的上下文 `IEffectSynthesisContext`；**失效判定与调度归宿主**（作用域信号保守标脏），处理器被调到 `Process` 就按当前真相干活（电平语义），跨调用复用内部缓存、自比缓存早退去重——「厚」在常驻缓存与私有重算图，不在失效上报。
...
- **厚处理器、失效归宿主**：`CreateProcessor(context)` 为「该 effect × 该上游段」建一个长生命周期实例、跨 `Process` 复用内部缓存；段销毁 / 删 effect / 重分段 / 换采样率时宿主 `Dispose`。宿主按作用域信号保守调度（自动化区间与本段的相交判定由宿主通用完成——改在别的段区间的编辑不会惊动本节点）；参数依赖/值级去重是引擎在 `Process` 内的可选早退（不重 Commit 即跳过下游）。
- **输入是整段不可分割的音频面**：`context.Input`（`IEffectSynthesisAudio`）暴露 `SampleOffset/Count/Rate` + `Read(offset, span)`（copy-out；宿主存储形态是实现细节）。没有「已提交」脉冲——被调到 `Process` 即输入已就绪（调度归宿主）；`Input.RangeModified(offset, count)` 是内容变更的**区间账本**（可选，区间平铺为参数对——与 automation `RangeModified` 同判例）：缓存型引擎累积区间、在自己成功产出后才清账（取消/失败不清）。
- **产出经握柄、登记表语义**：`context.CreateAudioSegment(offset, count, rate)`——产出分段自由（**1→N 合法**：如按静音切分的 **splitter**，为下游全体重建段粒度、免费获得按段增量与并行）；各输出段独立生灭，每个已提交段各自喂一个下游节点。输入面恒单段（消费单元 = 宿主的失效/调度/身份粒度）。唯一红线：**不得重新分布时间轴**（automation/回显与 part 显示共全局秒轴）；几何微差（补边、加尾）合法。采样率随段走（与工程率不同时宿主套一层重采样）。
...
- **回显轨（可选）**：引擎产出的只读曲线经 `GetSynthesizedParameterConfigs` 声明 + `IEffectSynthesisProcessor.SynthesizedParameters` 承载本段数据（宿主把同一 effect 各段按 key 拼接呈现）。只读、不可编辑、不进数据层、不序列化；与 voice 回显同构、在参数区标题栏按源显隐。
...
相关接口都在 `TuneLab.SDK`：`IEffectSynthesisEngine` / `IEffectSynthesisProcessor` / `IEffectSynthesisContext` / `IEffectSynthesisAudio` / `IAudioSegment` / `IEffectSynthesisPropertyContext` / `ISynthesisAutomation`。
