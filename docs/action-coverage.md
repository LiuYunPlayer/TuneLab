...
- **视口（缩放、滚动）**：模拟滚轮缩放仍然**不收**。它是以鼠标位置为轴心的连续手势（操作态，同上），
  而外部没有鼠标；且不改变任何结果，只改变用户此刻在看什么。同类的成熟远控面同样不暴露视口，可作旁证。
  **但这一条自己留下的那半句已经兑现**：它当时写着「真正有用的那件事是让用户看到我改了哪里，它要一个
  tick / 一个对象作参数——那是带参面的形状」，那就是 `command:editor reveal`（见 command-surface.md §6.1）。
  翻的是**定位**不是手势：手势没有参数、也不指向任何东西，而 reveal 恒要一个「哪里」。它在这张表里没有
  对应的界面入口——界面上"让某处进视野"是用户自己滚过去的，没有按钮。
