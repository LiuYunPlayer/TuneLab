...
- **全局分配语义（可用空间内统一分配，逐 note 独立、无单/跨两条路）**：每个 note 取可用空间 `[音符头 … 核填充末 FillEnd]`，收集落在其中的音素——本 note 非前置（核 + 后辅音），外加**仅当与后 note 相接时**后 note 的前置辅音——按三档统一分配：① 可用空间 ≥ 原长和 → **拉伸**，超出部分按权重摊给各核（w=0 不动；全 w=0 退化为按原长等比拉伸占满）；② 可用空间 < w=0 原长和 → **二级压缩**，核（w>0）全让到 0、辅音（w=0）按标称长等比压占满；③ 介于两者 → **一级压缩**，辅音保持原长，核按权重**水填**分配剩余空间（被压到 0 的核出局、余量在其余核间按权重重摊，直到无出局）。多核按各自权重**共同**让位（非单核独让）。例 `kas`+`bus` 相接：`a`（核）先让，耗尽再 `s`+`b`（辅音簇）一起等比压。**不变量：音素绝不溢出可用空间。**
...
- **核时长是分界基准、单核时被抵消**：核（`StretchWeight>0`）的 `Duration` 是其在可用空间内的原长——**多核**时决定相邻核的分界（余量按权重在各核原长之上分摊）；**单核**时原长被余量抵消、退化为填满核空间，报多少都一样。辅音（w=0）的 `Duration` 即其固定长。`IsLead` 决定前后归属（前置往左累积、核 + 后辅音往右）。因此输出**无需自己摆位**，只诚实报每个音素的时长 + 权重 + 前后标记即可，锁定时宿主按「核起点 = 音符头」统一派生位置——零跳变。
