...
> `IPartPropertyContext` 携带 **`VoiceId`**（选定声库）+ `PartProperties`（part 稀疏值）；`INotePropertyContext` 再加 `NoteProperties`。voiceId 进 context 使 voice 的 context 与 effect 的 `IEffectPropertyContext`（无此对等物）永久分叉——这是有意的：effect 是单类型引擎，没有「选哪个库」的概念。
