...
- **`IVoiceSynthesisPhonemeView`** (reads the phoneme's current values on the declaration side): `string Symbol` / `double Duration` / `double StretchWeight` / `PropertyObject Properties` (a snapshot of that phoneme's current property values). Before/after attribution is derived from the note view's `double Preutterance`, not a per-phoneme flag. You may condition the schema further based on these current values + position.
...
- **Reading values at synthesis time**: read from the snapshot — `VoiceSynthesisNoteSnapshot.Phonemes` is now `IReadOnlyList<VoiceSynthesisPhonemeSnapshot>`, each item `{ string Symbol; double Duration; double StretchWeight; bool IsLead; PropertyObject Properties }`. The geometry fields (`Symbol`/`Duration`/`StretchWeight`/`IsLead`) are read directly flat; when feeding `PhonemeLayout.Resolve` (which takes `SynthesizedPhoneme`) rebuild a `SynthesizedPhoneme` from the fields (see §5.7). `Properties` is that phoneme's frozen property values (unset = `PropertyObject.Empty`), read with `GetDouble(key, default)` etc., stored sparsely, falling back to the declared default when not found.
...
public sealed class VoiceSynthesisNoteSnapshot   // bottomed out to value types, no live reference whatsoever
{
    ...
    double Preutterance { get; }   // the note's pre-beat extent (pre-utterance, natural sec); before/after attribution derives from it
    IReadOnlyList<VoiceSynthesisPhonemeSnapshot> Phonemes { get; }   // frozen entries for pinned phonemes: each { string Symbol; double Duration; double StretchWeight; PropertyObject Properties } (geometry flat), see §5.7
...
The phoneme descriptor is **direction-agnostic** — read-in (the user's pinned constraints) and output (synthesis products) both use the same `SynthesizedPhoneme`: it reports only "nominal duration + weight", and **not absolute position, not before/after attribution**. Before/after attribution (pre-beat vs post-beat) is derived from a **per-note `Preutterance`** scalar, not a per-phoneme flag (this removes representable illegal states like an alternating lead flag). Positioning / cross-note de-overlap compression / melisma layout are all derived by the **host** via the same duration model (an engine reporting already-compressed absolute positions would make the host misjudge the adjacency criterion in its layout, so report only natural durations and let the host own layout exclusively).

```csharp
public struct SynthesizedPhoneme { public string Symbol; public double Duration; public double StretchWeight; }
```

**Input (host → engine): `note.Phonemes` (`IReadOnlyList<SynthesizedPhoneme>`, per note)**

- Represented as "duration + weight" (+ the note's `Preutterance`) rather than resolved absolute time — de-overlapping adjacent pinned notes (later-covers-earlier / **cross-note consonant-cluster compression**) must be solved jointly by the **global layout algorithm**; and using durations makes "push-style" editing easy (change one phoneme's length and the neighbors shift as a whole rather than crowding each other).
  ...
  - **`Preutterance` (per note, natural seconds)**: the amount of phoneme material sitting **before the note head** (the pre-beat / lead extent). It is where the note head falls on the phoneme sequence's natural timeline (offset from the first phoneme's start). Before/after attribution is **derived** from it: phonemes whose natural span lies before `Preutterance` are leading (pre-beat); a phoneme the note head cuts through **straddles** the beat (its front half goes to the previous note's compression domain, its back half to this note's). `Preutterance = 0` ⇒ nucleus-initial (no lead). To place N leading consonants at the boundary, set `Preutterance` = sum of their durations.
  - **Position is not stored, it is derived by layout**: the note head is the compression-domain boundary; leading material accumulates leftward from it (may cross before the note), the nucleus + trailing consonants go rightward — consonants use fixed durations, the nucleus fills to the **note's full end** (including melisma laid over passengers), and multiple nuclei split by weight.
...
**Pinning override**: `snapshot.Notes[i].Phonemes` non-empty = that note is user-pinned, so when materializing `PhonemeLayoutNote.Phonemes` use its pinned phonemes rather than your G2P prediction; only use the prediction when empty. Note that the snapshot's `Phonemes` elements are now `VoiceSynthesisPhonemeSnapshot` (no longer bare `SynthesizedPhoneme`), so rebuild the geometry fed to layout as `SynthesizedPhoneme` from `ph`'s fields (`{ Symbol = ph.Symbol, Duration = ph.Duration, StretchWeight = ph.StretchWeight }`), set the `PhonemeLayoutNote.Preutterance` from `snapshot.Notes[i].Preutterance`, and take per-phoneme properties from `ph.Properties` (see "Phoneme properties" in §5.2 and the end of §5.7).
...
// SynthesizedSyllable = { IReadOnlyList<SynthesizedPhoneme> Phonemes; double Preutterance; }
// Each attribution note → that note's phoneme set (reporting only Symbol / Duration / StretchWeight, no absolute position) + the note's Preutterance (pre-beat extent)
```
...
- **The nucleus duration is a base ratio, cancelled out when single**: a nucleus's (`StretchWeight>0`) `Duration` is its original length — with **multiple nuclei** it sets the base ratio between them (each multiplied by `r^w`); with a **single nucleus** the original length is cancelled out and it degenerates to filling the nucleus space, so whatever you report is the same. A consonant's (w=0) `Duration` is its fixed length. The group's `Preutterance` decides before/after attribution (material before it accumulates leftward, the rest goes rightward). Therefore the output **needs no positioning by you** — just honestly report each phoneme's duration + weight and the note's `Preutterance`, and the host derives positions uniformly — zero jumping.
...
public sealed class MyVoiceEngine : IVoiceSynthesisEngine, IExtensionSettings
{
    ...
    public void ApplySettings(PropertyObject settings)
    {
        ...
        // Then use these values in Init / CreateSession / CreateProcessor.
    }
