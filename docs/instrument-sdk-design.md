...
- `ILiveAutomation`（+ `IAutomationEvaluator`）、`SynthesisAutomationSnapshot`
- `SynthesizedParameter`（参数回显富类型）
- `SynthesisRange`（调度提示）、`SynthesisStatusSegment`（状态时间线）
- `AutomationConfig`（轨声明）

> 注：`IPartPropertyContext` / `INotePropertyContext` **不在**中性集——它们携带音源身份 id
...
- 与 `ILiveNote` 的差异：**`EndTime` 是满末、不钳位**（这是分水岭）；**无 `Lyric`、无 `Phonemes`**。
...
- 与 `ISynthesisContext` 的差异：**删 `Pitch` / `PitchDeviation`**（v1 纯 note pitch 发声）。
...
- 与 `ISynthesisSession` 的差异：**删 `DefaultLyric`、`SynthesizedPhonemes`、`SynthesizedPitch`**
...
- **automation**：连续 / 分段两形态由 `AutomationConfig.DefaultValue` 是否 NaN 区分；区间失效订阅
  `ILiveAutomation.RangeModified`；快照开窗冻结求值器。见 voice §7。
