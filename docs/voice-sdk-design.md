...
```csharp
public readonly struct Position { public double Tick { get; } public double Second { get; } }
...
**命名约定**：`Tick` = 乐理位置（单位 tick），`Time`/`Second` = 实时位置（单位秒）。**单复数表元数**：表示单个时间点用单数（`Position.Tick`/`Position.Second`、`ToTick`/`ToSecond`），批量才用复数（`ToTicks`/`ToSeconds`）——不沿用 .NET 的"单值也复数"惯例（`DateTime.Ticks`），元数从名字直接可读。新 SDK 一律用 `Position`，杜绝歧义的 `Pos`。宿主全局 `Pos → Tick` 重命名是好清理但量大、且可能触及 Format 序列化 AB（`NoteInfo.Pos` 等），作为**独立 refactor**、本设计不捆绑。
