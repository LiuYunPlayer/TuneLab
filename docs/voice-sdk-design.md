...
automation **开窗**只取该段区间的原始锚点，不是整条曲线整 part 拷。**无变形开窗规则**：取闭区间 `[start, end]` 内的锚点 + 每侧**开区间之外**最近的至多两个锚点（边界恰为锚点时其自身计入）。两个是单调 Hermite 的斜率影响半径——查询所落段两端锚点的斜率各依赖再向外一个锚点，外扩两个恰好补齐，窗口内取值与全曲线**逐点全等**；查询恰落在锚点上时不依赖斜率，故压边界一侧只需外扩一个。注意不能用"边界处采样固定端点值"的取段方式（如编辑用的 `RangeInfo`），那会改变边缘段的插值形状。
...
// 起始与长度创建时声明；几何变更分两种意图：语义整改换新 → 删旧建新（下游身份重建）；
// 内容延续的扩展/裁剪 → Resize（身份保持，下游缓存存活）。
// 时间对齐协议：全局 0 时刻 = 采样点 0；缓冲按 native 采样率从段起始铺。
// 线程：写入 / 提交 / 改几何 / 释放全在数据线程（worker 渲染完，在 marshal 回数据线程的续延里写）。
public interface IAudioSegment : IDisposable   // Dispose() = 删除该段（语义整改换新时重建）
{
    ...
    // 宿主记账写入区间（绝对采样位置），随 Commit 传导下游（effect 的 Input.RangeModified 账本）。
    ...
    // 就地改几何、身份不变：交集内容按全局采样位置对齐保留（内容钉绝对轴、前/后向扩展对称），
    // 新增清零、回未提交态（写完新增区重 Commit 收口，期间下游持续消费上一版已提交快照）。
    // 用例：inpainting 引擎在内容首/尾新增 note——只补渲新增区，下游 effect 节点与缓存全程存活。
...
- **固定（`SynthesisLock`）**：把产物在给定范围内按**真实数值**写成用户曲线的锚点，一次可撤销命令。入口 = **固定笔刷**（`Locking Brush`，原生工具）+ 颤音右键「固定此区间的模型输出」（把动作放在最需要它的位置）；曾加过的范围选区右键菜单已删——与笔刷重复，将来整轨/选区作用域交给脚本面暴露成动作，让用户自己决定入口位置（`WholeTrackStart/End` 常量为此保留）。两种载体——参数区的配对回显 → 活动轨、音符区的合成音高 → `Pitch`，同一份实现（旧 `MidiPart.LockPitch` 已合流进来）。这是"抓住模型这根线只改中间一段"的手段——没有它，用户在未覆盖段落笔就是从空白起步、模型细节全丢。**与音素钉死（`INote.LockPhonemes`）是同一范式**：引擎产物 → 归用户的可继续编辑数据，故术语统一用"固定 / lock"（中文在音素侧沿用"钉死"，指同一件事）。
  **刻意只做显式一次性动作**：自动跟随会形成 用户覆盖 → 重合成 → 回显变 → 再写入 的反馈环，且把合成结果混进 undo 栈。
  三条如实性：① 秒→tick 换算即固化（固定后改 tempo 不再跟随模型输出，与手画曲线同待遇）；② 值不按目标轨量程钳位（钳位会静默改数据），量程声明不一致的后果即出界；③ **写入前扣掉该轨的 vibrato 偏移**——产物是"用户曲线 + 偏差"的结果，原样写回会让重合成再叠一次偏差，每固定一次颤音幅度就翻一圈（这曾是 `LockPitch` 的致命 bug）。扣减使固定**幂等**：写入后最终求值恒等于当次产物，反复固定同一段结果不变；扣减在简化之前（先减成平滑基线再简化，锚点数远少于对含颤音的震荡曲线简化）。pitch 走 `GetPitchVibratoDeviation`、连续轨走 `GetVibratoDeviation(key)`、分段轨不参与 vibrato 故不扣。
...
- **"编辑态 vs 自动"的可视化**：哪段实 / 哪段虚，宿主按"该 key 可编辑轨用户是否动过"自己决定渲染，**插件不报透明度**（曾有把逐段透明度 stop 塞进产物的提案，被否：透明度是纯渲染量、且用相对位置 0~1 违反全秒轴；插件只供语义数据，渲染策略归宿主）。
...
*阶段 C — 回显可编辑（约定，SDK 零改动）* —— **已落地**（细则见 §7「回显轨 ⇄ 可编辑轨」）
- ✅ 配对判据 = 同 `PropertyKey.Id`（`DisplayText` 各自自由，支持「张力 / 张力（实参）」式命名分层）；量程不作要求、两边各自投影（不一致即显示错位，作者自见自改）。
- ✅ 显隐单向联动：点亮可编辑轨即带亮同 key 回显（只带亮、不连带熄灭、不锁 chip、一次性）。回显保留独立 chip 与显示名，渲染零改动（图层序本就把回显放在活动轨之下）。
- ✅ 固定 `SynthesisLock`（术语与音素钉死同一范式，见 §7）：回显 / 合成音高 → 目标轨，按真实数值、选区或整轨、一次可撤销。两个入口 = 范围选区右键菜单 +**固定笔刷**（`Locking Brush`，原「Pitch Locking Brush」去掉 Pitch 限定——它现在也作用于参数轨）。显式一次性、不自动同步。
- ✅ 旧 `MidiPart.LockPitch` 合流进 `SynthesisLock`（此前是并存的第二份实现）：统一秒→tick 换算、边界插值裁剪、简化口径。
...
- ✅ **可编辑分段轨三侧接线补齐**（配对 / 固定链路的载体，此前只有 voice 半通）。一条完整链需三环：① 活视图 map 含该轨（引擎才能订阅——三侧本就按**声明集**建代理，形态透明）② `RangeModified` 转发（引擎才知道要重算）③ 快照物化读分段数据（引擎才取到真值）。补前状态：voice 缺 ②③、effect 缺 ②、instrument 缺 ②③。
  - voice：`VoiceSynthesisContext.WirePiecewiseAutomation` + `VoiceSynthesisSnapshotFactory` 按 `IsPiecewise` 分派。
  - effect：`EffectGraph.WireAutomations` 加订 `PiecewiseAutomations`（`MapModified` + 各条 `RangeModified`）；快照侧本来就对。
  - instrument：`InstrumentSynthesisContext` 加分段接线（另开一张簿记表）+ `InstrumentSynthesisSnapshotFactory` 按 `IsPiecewise` 分派。
  - 两处共同口径：**簿记表与连续轨分开**（同一 key 换过形态后两张数据 map 可能各有对象，共用一张表会让先接线的把另一条挡在门外）；分段轨**无 `DefaultValue` 通道**故只订 `RangeModified`；无数据对象时冻结为 **NaN 常量**（不是 0，分段轨的"无值"就是 NaN）。
...
- **音频段内子区间增量**（`IAudioSegment` 段内增量）——**已落地**：宿主累积 `Write` 区间（绝对采样位置、跨采样率缩放），随 `Commit` 经 effect 的 `Input.RangeModified` 账本传导；effect 侧局部重合成为可选能力（账本收窄重算窗、成功产出清账、输出几何变更走 `Resize` 保身份——参考实现见 V1.Effect 的 Slow Gain），无局部能力的引擎整段重跑仍合法。上下文余量 / 淡化归引擎自决。
- **effect 收敛到本会话模型**（当前 effect 为「宿主厚 / 插件薄」的逐段处理器模型：状态/调度/`cache[segment][stage]` 失效图全在宿主）。**设计已铺细 → §10**；落地分阶段见 §10.8。已锁的收敛决策（5 条，§10 逐条展开）：① 平行接口 `IEffectSession`（不与 `IVoiceSynthesisSession` 合并基类）；② `IEffectSynthesisContext` 对偶 `IVoiceSynthesisContext`（暴露上游音频段活视图 + 自身参数/自动化 + GetSnapshot + 产出段，宿主接线、effect 对链结构无感）；③ 调度统一、链为单位（一个段区间自上而下跑完一遍 = 一个调度单位，按播放线就近挑；effect-only 改动退化为从脏的那一级往下）——voice 非音频产物（pitch/phoneme）不依赖 effect、eager 暴露不被串行；此为宿主调度策略、不进冻结面；④ `IEffectChange` 退役，effect 订阅 context 自算 dirty（失效图搬进插件）；⑤ `SynthesizedParameters` 借此换富类型（见下）。重构时一并处理：
...
- ~~**动态声明面**：轨集合/属性声明运行中变化 + 既有轨用户数据的归宿~~（**已实现**：声明全部 context 驱动、纯函数，
  ...
  收 `IVoiceSynthesisPartPropertyContext`），属性面板同 `GetPartPropertyConfig`/`GetNotePropertyConfig`；effect 侧 `IEffectSynthesisEngine.GetPropertyConfig`/`GetAutomationConfigs` 同构（各收 effect 专属 `IEffectSynthesisPropertyContext`——voice/effect context 分开以备 effect 将来追加 part 级官方字段而发散；effect 单层故用不带 Part 的 `GetPropertyConfig`）。**（本轮再修订见 §8：voice context 由快照改活视图薄壳、VoiceId 并入 `IVoiceSynthesisContext`、加 `GetPhonemePropertyConfigs`；上述 commit-重算-diff 机制不变。）**
...
当前 effect 是**宿主厚 / 插件薄**的逐段处理器：宿主（`VoiceSynthesisPipeline`）全包了 `cache[segment][stage]` 失效图、脏判定、变化事实构造（`IEffectChange`）、段间串行调度、`input[i]=output[i-1]` 链接线；插件只看到 `IEffectSynthesisSession.Process(整段 input, output, change)`。本节把它收敛到**厚 processor / 每段一个**——保留 `IEffectSynthesisSession` 概念（不另立会话），但让它变厚：持有自己那一段的上下文，自管这一个 segment 生命周期的失效与重处理。

> 与早期"对偶 voice 会话（`IEffectSession` + 多段 `IEffectSynthesisContext` + `GetSnapshot` 快照）"的设想相比，本模型刻意更简：**段间彼此无共享上下文**（音频段间共享上下文意义不大，分别处理后再混音才是多数算法更正确的做法），故无需会话去统管多段、无需快照/开窗机制。早期 §9 列的决策 ①②（平行 session / 对偶 context）被本模型覆盖；③（链为单位调度）④（自算 dirty）⑤（`SynthesizedParameter`）不变。

### 10.1 模型与退役

```
IEffectSynthesisEngine                每"effect 类型"一个：声明参数/自动化、创建处理器
  └ IEffectSynthesisSession          每「effect 实例 × 一个上游音频段」一个：持有本段上下文，自管该段失效与重处理
```

- `IEffectSynthesisEngine.CreateSession()` → **`CreateSession(IEffectSynthesisContext context)`**（context 绑定该 effect × 一个上游段）。
- **退役**：`IEffectInput` / `IEffectOutput` / `IEffectChange`（其职责并入下面的 `IEffectSynthesisContext` 活视图 + processor 自管失效）。
- **段间独立、无共享上下文**：每个 processor 只看自己那一段，对链结构无感；宿主把 effect[i] 的输出段接成 effect[i+1] 的输入段。

### 10.2 输入：`IEffectSynthesisContext`（每 processor / 每段）

宿主实现、绑定「该 effect × 一个上游音频段」、随 processor 死。失效判定与调度归宿主，
context 的颗粒事件只是缓存型引擎的可选信息源。

```csharp
public interface IEffectSynthesisContext
{
    // 本段输入（只读音频面 IEffectSynthesisAudio：SampleOffset/Count/Rate + Read 区间 copy-out +
    // RangeModified 内容变更区间账本）。Process 被调到即输入已就绪，无「已提交」脉冲。
    IEffectSynthesisAudio Input { get; }

    // 该 effect 自身参数活视图（同步前缀读值物化；Modified 为可选缓存提示）。
    IReadOnlyNotifiablePropertyObject Properties { get; }

    // 该 effect 声明的连续自动化轨（按 key；查询轴 = 全局秒）：只读 map（分段轨不在此列）。
    // RangeModified 为可选缓存提示（调度所需的区间相交判定由宿主完成）。
    IReadOnlyMap<string, ISynthesisAutomation> Automations { get; }

    // 产出（与 voice 同一登记表语义、同一握柄 IAudioSegment）：产出分段自由（1→N 合法，如 splitter）；
    // 唯一红线 = 不得重新分布时间轴。宿主把每个已提交输出段接成下游 effect 的输入。仅数据线程调用。
    IAudioSegment CreateAudioSegment(long sampleOffset, int sampleCount, int sampleRate);

    // 跨线程冻结快照（仅同步前缀；参数值 + 自动化冻结求值器；音频不在此列——Input.Read 自物化）：
    // worker 自定采样时刻时用；前缀可枚举时刻的简单引擎直接对活视图 Evaluate 成数组即可。
    EffectSynthesisSnapshot GetSnapshot(double startTime, double endTime);
}
```

**为何 copy-out 而非直读引用**：读取是「区间 + 调用方缓冲」——插件心智模型是随机访问一个完整数组，
但永远拿不到宿主内部存储的引用，宿主可无缝演进分页存储（解决长段的重分配频率），线程故事也更简单
（worker 只碰同步前缀拷出的自有缓冲）。自动化值由 processor 在同步前缀用 `ISynthesisAutomation.Evaluate`
（数据线程）预采成数组再 offload；参数同理读值。都是 processor 自管的轻量捕获，不需要 SDK 快照类型。

### 10.3 `IEffectSynthesisSession`（厚）

```csharp
public interface IEffectSynthesisSession : IDisposable
{
    // (重)处理本段。电平语义：让输出与当前输入一致（非「应用某次变更」）；宿主保守调度，
    // 引擎自比缓存早退去重（不重 Commit 即跳过下游）。返回语义同 voice SynthesizeNext：
    // 纯 Task 无 outcome；取消正常返回（不抛 OperationCanceledException）；await 真正返回 = 槽位释放；
    // 错误抛异常，宿主在调用边界 catch → 该段 passthrough 降级（状态带琥珀 Degraded）。
...
- **无失效上报面**：`ProcessingRequested`/`Committed` 已退役——失效判定权归宿主（作用域信号：本段输入
  重 Commit / 本 effect 参数 settled 变更 / 自动化变更区间∩本段），处理器零订阅即可正确工作。
- **状态呈现**：宿主以画家算法分层绘制（声称完成垫底 → 音频事实 Final/Interim/Degraded → 活动声称盖顶），
  亮绿只能来自链尾音频事实；不自报状态的引擎由宿主按调度事实兜底。

### 10.4 失效自管（决策 ④：`IEffectChange` 退役）

原 `IEffectChange`（= `VoiceSynthesisPipeline.StageChange`）由宿主构造"自上次变了什么"递给插件；现搬进 processor：processor 订阅 `IEffectSynthesisContext` 自算 dirty——`Input.Committed` → 输入变；`Properties.Modified` → 参数变；`automation.RangeModified` → 该轨该秒区间变（按本段时间界自筛）。脏 → 触发 `ProcessingRequested` → 宿主调度 `Process`。原宿主的 `cache[segment][stage]` 二维失效图就此**分解进各 processor**（每个 processor 管自己那一格的失效与重处理），宿主不再算变化事实。无内部增量的引擎仍可"任何信号 → 整段重处理"。
...
- **阶段二（effect 实现，一体 chunk，已完成）**：新 `IEffectSynthesisSession`（厚）/ `IEffectSynthesisContext`（每段）/ `IReadOnlyAudioSegment` + 退役 `IEffectInput`/`Output`/`Change` + `IEffectSynthesisEngine.CreateSession` 改签名 + 宿主 effect 半部重写为「effect × 段」反应式 processor 图（`EffectGraph`：voice 段 / 各级输出段为图中输入，每节点一厚 processor 自管失效；reconcile + pump、跨段并行受 `Settings.MaxParallelSynthesisTasks` 全局闸门封顶；链尾各输出段经消费端按绝对时间混音）+ 夹具 `V1.Effect` 重写。compat 无 effect 适配（无需同步）。这些改 `IEffectSynthesisSession`/`IEffectSynthesisEngine` 签名、必须一起落才编译绿，故为一体 chunk（不可像 voice 侧那样纯 additive）。
