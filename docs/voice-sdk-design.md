...
public interface ISynthesisNote
{
    ...
    IReadOnlyNotifiableProperty<IReadOnlyList<PhonemeInfo>> Phonemes { get; }  // 见 §6
...
| 数据 | 形态 | 进程内 | 跨进程（v2） |
|---|---|---|---|
| **note** | eager 物化的不可变值快照（`StartTick/EndTick` 全局 tick——秒经 `snapshot.Timing` 换算、`Pitch`、`Lyric`、`PhonemeInfo[]`、`Properties` 值拷；有序列表与递入 notes 索引对齐，邻居按索引导航） | worker 直读 | 序列化进消息体送过去 |
...
现成接口 `double[] GetValue(IReadOnlyList<double> times)` **无需改**：插件递一列时间点、拿回一列值。背后实现按 transport 换——进程内直读冻结点插值，跨进程是批量 RPC。插值算法永远单一权威地留在宿主侧，插件（含未来插件进程）不需要懂插值。跨进程务必**批量**（一次递一段所需全部 times），别逐点 RPC。
...
- **note 快照值**（StartTick/EndTick、Pitch、Lyric、`PhonemeInfo[]`、Properties 值拷；有序列表索引对齐、不带邻居链）；automation 按无变形开窗规则（见 §3.5）取原始锚点。
