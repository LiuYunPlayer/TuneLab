...
**为何取消 `IVoiceSource`**：它原本承载"某个声源的声明 + 合成任务工厂"。现声明直接挂在会话上、合成由会话本身承担；且声源对象本就是每 part 一份（不共享），与会话一一对应。曾担心的"构造顺序循环"（part 的自动化轨需要 schema、schema 在会话上、会话又依赖 part 数据）不成立：声库元数据由 `VoiceInfos` 目录在创建会话前就提供；自动化等 schema 的*取值默认*由插件自己用其声明填充（宿主不需要 schema 即可构造 context），schema 仅供 UI 在会话创建后消费；且任何"有声源的 part"必有会话（会话是轻量句柄，重模型加载是懒的）。
...
{
    IReadOnlyNotifiableProperty<double> StartTick { get; }   // 全局 tick（乐谱量的真值域）
...
**时间真值域原则：每个量只在它的真值域出现一次，`ITiming` 是两域之间唯一的桥。**乐谱侧输入（note 边界、曲线锚点、开窗区间）真值是 tick——tick 不随 tempo 漂；声学/产物侧（音频、`SynthesizedPhoneme`、状态段、`SynthesisSegment`、钉死音素的 note 相对秒）真值是秒。需要另一域时经 `context.Timing`（活视图）/ `snapshot.Timing`（冻结面）显式换算，通常在 `SynthesizeNext` 同步前缀批量一次算完。不设双域成对的 `Position{Tick,Second}`：派生值不进契约（同 `TempoMark` 不带秒的理由），也消掉"秒分量随 tempo 漂、但属性自身不通知（信号走 `TimingModified`）"的隐性规则：

```csharp
public interface ITiming   // 任意时间点的换算（自动化采样点、音素边界等）
{
...
**Timing 不叫 Timeline**：timeline 是"承载事件的轴"（容器语义，DAW 语境专指编排视图），本接口不含内容、只是 tick 轴与秒轴之间由 tempo map 决定的双射度量（轴上的刻度，业界惯名 TimeMap/TempoMap）；`Timeline` 一名保留给将来宿主真正的时间线概念（拍号、小节标尺等）。

tempo 变化是 context 上的一条变更（`TimingModified`）：tick 真值不动，全部秒域派生作废，插件重新换算。

**命名约定**：`Tick` = 乐理位置（单位 tick），`Time`/`Second` = 实时位置（单位秒）。**单复数表元数**：表示单个时间点用单数（`StartTick`/`EndTick`、`ToTick`/`ToSecond`），批量才用复数（`ToTicks`/`ToSeconds`）——不沿用 .NET 的"单值也复数"惯例（`DateTime.Ticks`），元数从名字直接可读。新 SDK 杜绝歧义的 `Pos`。宿主全局 `Pos → Tick` 重命名是好清理但量大、且可能触及 Format 序列化 AB（`NoteInfo.Pos` 等），作为**独立 refactor**、本设计不捆绑。
...
| 数据 | 形态 | 进程内 | 跨进程（v2） |
|---|---|---|---|
| **note** | eager 物化的不可变值快照（`StartTick/EndTick` 全局 tick——秒经 `snapshot.Timing` 换算、`Pitch`、`Lyric`、`PinnedPhoneme[]`、`Properties` 值拷；有序列表与递入 notes 索引对齐，邻居按索引导航） | worker 直读 | 序列化进消息体送过去 |
| **automation** | **host 侧不可变原始点快照**；插件经 `IAutomationValueGetter.GetValue(times)` 拉采样值 | worker 直接调 getter、宿主插值算法就地对冻结点插值 | 快照序列化时物化为离散点（提前采样，跨进程牺牲项；细节缓后）；**插值算法恒在宿主侧** |
...
#### `IAutomationValueGetter.GetValue(times)` 是传输无关接缝

现成接口 `double[] GetValue(IReadOnlyList<double> times)` **无需改**：插件递一列时间点、拿回一列值。背后实现按 transport 换——进程内直读冻结点插值；跨进程（基于 v1 接口做，本就有牺牲项）在快照序列化时把取值器物化为离散点（细节缓后）。插值算法永远单一权威地留在宿主侧，插件不需要懂插值。
...
- **统一命名根 `Automation`**（不用业务词 `Expression`，也不用纯结构词 `Curve`）：冻结的取值器 `IAutomationValueGetter` 已把"Automation"锚定为"按时间取值"的伞概念，且 `IAutomation` 本就把曲线几何操作收在该名下。故 host 数据层 `IPiecewiseCurve / PiecewiseCurve` 改名为 `IPiecewiseAutomation / PiecewiseAutomation`，pitch 直接 typed 为 `IPiecewiseAutomation`，删 `PitchAutomation.cs` 空壳。
- **取值器统一、NaN 表空**：连续型与分段型共用 `IAutomationValueGetter.GetValue(ticks) → double[]`；连续型永不返回 NaN，分段型段间返回 NaN（IEEE 标准"非数"，DSP 惯用，且现有 pitch getter 与 ACE 都这么做）。

---

## 8. 声明数据与 Config 家族

- **目录元数据**在 `IVoiceEngine.VoiceInfos`（菜单/选择器用，无需会话）：
...
- **note 快照值**（StartTick/EndTick、Pitch、Lyric、`PinnedPhoneme[]`、Properties 值拷；有序列表索引对齐、不带邻居链）；automation 按无变形开窗规则（见 §3.5）取原始锚点。
