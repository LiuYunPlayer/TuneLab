...
### 3.3 `ISynthesisNote` / `Position` / `ITiming`
...
**`Position`：同时给 tick 与秒**。某些引擎只需秒、某些需要精确 tick 时钟：

```csharp
public readonly struct Position { public double Tick { get; } public double Seconds { get; } }
...
tempo 变化是 context 上的一条变更（位置随之 re-derive）。

**命名约定**：`Tick` = 乐理位置（单位 tick），`Time`/`Seconds` = 实时位置（单位秒）。新 SDK 一律用 `Position`，杜绝歧义的 `Pos`。宿主全局 `Pos → Tick` 重命名是好清理但量大、且可能触及 Format 序列化 AB（`NoteInfo.Pos` 等），作为**独立 refactor**、本设计不捆绑。
...
- **`WillModified` 改前事件**：作废"被腾空的旧区域"必需。note 从 A 移到 B，`Modified` 只给 B（当前值），`WillModified` 给 A（旧值），插件据此把 A、B 两段都作废。这要求宿主数据层（DataObject↔NotifiableProperty 统一）补上 `WillModified`。
