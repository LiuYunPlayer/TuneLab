...
public sealed class SynthesisSnapshot
{
    ...
    public required IAutomationEvaluator Pitch { get; init; }                 // 冻结求值器（全局 tick 轴），开窗 = 拉取区间；双通道语义同活视图
...
- **求值器统一、NaN 表空**：连续型与分段型共用 `IAutomationEvaluator.Evaluate(points) → double[]`（voice 面查询轴 = 全局 tick）；连续型永不返回 NaN，分段型段间返回 NaN（IEEE 标准"非数"，DSP 惯用，且现有 pitch 求值与 ACE 都这么做）。
...
- 不可变**原始点快照容器** + 在其上的不可变 `IAutomationEvaluator` 实现（`Evaluate(points)` 对冻结点插值）。
