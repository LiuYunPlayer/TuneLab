...
- **阶段二（effect 实现，一体 chunk）**：新 `IEffectProcessor`（厚）/ `IEffectContext`（每段）/ `IUpstreamAudioSegment` + 退役 `IEffectInput`/`Output`/`Change` + `IEffectEngine.CreateProcessor` 改签名 + 宿主 `VoiceSynthesisPipeline` effect 半部重写（processor 图 / 链接线 / 链为单位调度 / 混音）+ 夹具 `V1.Effect` 重写 + compat。这些改 `IEffectProcessor`/`IEffectEngine` 签名、必须一起落才编译绿，故为一体 chunk（不可像 voice 侧那样纯 additive）。
