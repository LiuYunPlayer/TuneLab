...
- 属性约定（gap：note/part props）：per-note/part 专属参数全走 keyed `Properties`（不动 `IVoiceSynthesisNote` 固定面）——`GetNotePropertyConfig`/`GetPartPropertyConfig` 的 `ObjectConfig.Properties`（`OrderedMap<string, IControllerConfig>`）声明，合成时从 `VoiceSynthesisNoteSnapshot.Properties`/`snapshot.PartProperties` 用 `GetDouble/GetBool/GetInt/GetString/GetEnum(key, default)` 读（稀疏、读不到取默认）。控件：`SliderConfig`/`ComboBoxConfig`(值显分离)/`CheckBoxConfig`/`TextBoxConfig`。`AutomationConfig.DefaultValue=NaN⇒分段轨`；`AutomationConfig.Create(INormalizedScale)` 可传自定义标度（`NormalizedScale.Integer(min,max)`=整数轨）。**离散标度⇒信号处处落格**：宿主在求值+渲染把 Hermite 连续输出投影回标度（曲线阶梯、`Evaluate` 返回落格最终值、覆盖 load/preset/回喂），引擎无需自行取整；线性标度投影=纯范围钳位。条件轨消失后宿主保留已画曲线（隐藏不删）。`VoiceSourceInfo.Portrait` 用 `FileImageResource(绝对路径)`。
