...
> voice = **会话托管厚模型**：每种引擎一个 `IVoiceEngine`；宿主为每条 MidiPart 建一个 `IVoiceSession`，
...
public interface IVoiceEngine {
    ...
    void Destroy();
    IVoiceSession CreateSession(string voiceId, IVoiceContext context);   // 每 part 一个会话；context 随会话同生死
    ...
    IReadOnlyOrderedMap<string, AutomationConfig> GetAutomationConfigs(IVoicePartPropertyContext context);            // 自动化轨（连续/分段同一 map，DefaultValue=NaN ⇒ 分段）
    IReadOnlyOrderedMap<string, AutomationConfig> GetSynthesizedParameterConfigs(IVoicePartPropertyContext context);  // 只读回显轨声明（分段形 DefaultValue=NaN）
...
public interface IVoiceSession : IDisposable {
    ...
    IReadOnlyMap<IVoiceNote, IReadOnlyList<VoicePhoneme>> SynthesizedPhonemes { get; }  // 按归属 note 键（origins[i]）；只报时长、无绝对位置；无主音素无契约
    IReadOnlyList<SynthesisStatusSegment> GetStatus();                         // 按段状态/进度/报错
    event Action? StatusChanged;                                              // 产物/状态有更新（任意线程触发，宿主 marshal）
}
public interface IVoiceContext {        // 会话级输入活视图（宿主实现、随会话死、仅数据线程访问）
    IReadOnlyNotifiableLinkedList<IVoiceNote> Notes { get; }   // 可重叠（和弦）、排序 StartTime↑→EndTime↓→插入序；去重叠是插件责任
    ...
    VoiceSnapshot GetSnapshot(IReadOnlyList<IVoiceNote> notes, double startTime, double endTime);  // 仅 SynthesizeNext 同步前缀（offload 前）主动拉，可拉多份
...
```csharp
public sealed class VoiceSnapshot {                    // context.GetSnapshot(notes, startTime, endTime) 的返回体
    IReadOnlyList<VoiceNoteSnapshot> Notes { get; }    // 与递入 notes 【索引对齐】（产物归属契约）；邻居按索引导航、无 Next/Last
...
public sealed class VoiceNoteSnapshot {                // 触底值类型、无活引用
    double StartTime { get; } double EndTime { get; }      // 全局秒
    int Pitch { get; } string Lyric { get; }
    bool IsContinuation { get; }                       // 宿主稳定标志=「生效延续」(延音符∧相接链回溯到发声note;孤儿延音符=false)；直接信它判延续,勿匹配记号、不必自判相接。活视图 IVoiceNote.IsContinuation 同型(普通只读字段、无通知;输入 Lyric/位置可订阅)
...
- 声明在引擎（gap：declaration timing）：4 个 GetConfig 在 `IVoiceEngine` 上、是 `f(voiceId, 选中成员当前值)` 纯函数。`IVoicePartPropertyContext { string VoiceId; IReadOnlyList<PropertyObject> PartProperties }`（part 面板，可多选 part；多声库经 `VoiceId` 分流）。`IVoiceNotePropertyContext`**独立不继承** `{ string VoiceId; PropertyObject PartProperties; IReadOnlyList<PropertyObject> NoteProperties }`——note 必属单 part 故 PartProperties 单数、NoteProperties 是各选中 note 列表。列表成员不在乎多选就 `context.NoteProperties.Merge()`（`PropertyObjectExtensions` 扩展方法@`TuneLab.Foundation`）还原成单个三态 `PropertyObject` 按单选写；要逐成员真值就遍历列表。voiceId 进 context 使 voice 的 context 与 effect 的 `IEffectPropertyContext`（无对等物）永久分叉。**会话构造期即可订阅自己声明的轨**：宿主在建会话前已据引擎声明填好轨集合，故 `context.TryGetAutomation(key,…)` 对你声明过的轨必成；漏订则该轨绘制后不触发重渲。
- 调度语义：一会话同时只合成一块；并行发生在不同 part 的不同会话间，并发上限由宿主管控。取消是正常调度结局（不抛 `OperationCanceledException`）；`await` 真正返回才释放槽位。
- 线程纪律：context（Notes/属性/automation）、`GetSnapshot`、`CreateAudioSegment` 仅可在 `SynthesizeNext` **同步前缀**（数据线程）读/调；之后 offload 只读已物化的 `VoiceSnapshot`（不可变、可跨线程）。产物与 `CreateAudioSegment` 写入/Commit 在数据线程。
- 命名纪律：活视图类型（`IVoiceContext`/`IVoiceNote`/`ISynthesisAutomation`）仅数据线程访问；`*Snapshot`=冻结物（可跨线程、无事件）。
...
- 音素（gap：phoneme I/O）：进/出**同一类型** `VoicePhoneme`（只报 Symbol/Duration/StretchWeight/IsLead，**无绝对位置**——定位/去重叠/melisma 全归宿主派生）。输入 `note.Phonemes` 整note钉死（非空=全钉、引擎守约束；空=从 Lyric 做 G2P+自由定时；不支持部分钉死）。输出 `SynthesizedPhonemes` = **按归属 note 键的 map**（键=按快照索引从你递入的活 note 列表 `origins[i]` 回取，仅身份token、合成中不读其属性）；脏/合成中的块不报其 note 音素（宿主留白）。**无「无主音素」契约**（时长模型下无锚不可定位）。核时长由宿主填充派生（报多少无所谓）、辅音填固定长；`StretchWeight` 无知识填 `Duration`。preview 纯显示、绝不当约束回喂。要把标称时长解析成真实时序驱动音频帧（避免重叠双算撑长），调 SDK 纯函数 `VoicePhonemeLayout.Resolve(IReadOnlyList<VoicePhonemeLayoutNote>)`（`VoicePhonemeLayoutNote{ FillStart=音符头, FillEnd=你算的前向铺末, Phonemes }`，返回同构交错数组 `VoicePhonemeTiming[][]`{Start,End,Duration}，`result[i][j]`=落点）——与宿主显示同一份代码、WYSIWYG、随宿主演进不漂移。**只接管定位/去重叠;标称时长(G2P/分组/dur 模型/padding)仍引擎专属、不被消掉。** `FillEnd` 取法：要 WYSIWYG 须与宿主同口径=自己有效末 + 仅延续乘客(IsContinuation) melisma、真发声 note 间空隙停在自己末(不铺过)。「错位非致命」只对**纯显示**成立——用 Resolve 驱动音频时 FillEnd 偏离口径(如填过空隙)→ 音频与显示分叉、**听得见**。
- 属性约定（gap：note/part props）：per-note/part 专属参数全走 keyed `Properties`（不动 `IVoiceNote` 固定面）——`GetNotePropertyConfig`/`GetPartPropertyConfig` 的 `ObjectConfig.Properties`（`OrderedMap<string, IControllerConfig>`）声明，合成时从 `VoiceNoteSnapshot.Properties`/`snapshot.PartProperties` 用 `GetDouble/GetBool/GetInt/GetString/GetEnum(key, default)` 读（稀疏、读不到取默认）。控件：`SliderConfig`/`ComboBoxConfig`(值显分离)/`CheckBoxConfig`/`TextBoxConfig`。`AutomationConfig.DefaultValue=NaN⇒分段轨`。条件轨消失后宿主保留已画曲线（隐藏不删）。`VoiceSourceInfo.Portrait` 用 `FileImageResource(绝对路径)`。
...
- 相关类型：`IVoiceSession`、`IVoiceContext`、`SynthesisRange`、`VoiceSnapshot`、`VoiceNoteSnapshot`、`SynthesisAutomationSnapshot`、`IAutomationEvaluator`、`IVoiceNote`、`ISynthesisAutomation`、`IAudioSegment`、`VoicePhoneme`、`VoicePhonemeLayout`、`VoicePhonemeLayoutNote`、`VoicePhonemeTiming`、`SynthesizedPitch`、`SynthesizedParameter`、`VoiceSourceInfo`、`FileImageResource`、`AutomationConfig`、`IVoicePartPropertyContext`、`IVoiceNotePropertyContext`、`SynthesisStatusSegment`。
...
public interface IEffectContext {        // 本段输入上下文（宿主实现、随处理器死、仅数据线程访问）
    ...
    bool TryGetAutomation(string key, out ISynthesisAutomation automation);   // 该 effect 一条自动化轨活视图（查询轴=全局秒）
