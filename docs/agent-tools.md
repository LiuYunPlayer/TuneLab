...
```
（列引擎/config 文本化的共享格式在 `AgentToolFormat.cs`：`EngineCatalog.AppendEngineList` 三类引擎共用、`ConfigText.Describe/FormatValue` 各 config 类型一致措辞，也复用进脚本入参 `SavedScriptSupport`。）
