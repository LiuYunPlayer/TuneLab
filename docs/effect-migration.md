...
**伏笔（另列未来查漏补缺，不在本次）**：IEvent 框架把事件抽象为可赋值/存储/传递的一等值（现 `IEvent<Action<T>>` 委托形状 vs Rx 式 `IObservable<T>` 值形状）+ `When`/`Any` 组合子——方向正确（C# 原生 `event` 非一等公民，须对象包一层，同 Rx）；`AnyEvent` 的 bug 与组合子要手搓，是"委托形状不好组合"的证据。该专题与条件表达式（§三.13 推迟项）天然同源，单独再谈。
