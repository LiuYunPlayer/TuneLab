...
- 值为 [KeyCodec](../TuneLab.GUI/GUI/Input/KeyCodec.cs) 令牌串（如 `"mod+shift+k"`），经 `KeyCodec.TryParseDeclaration`
  ...
  **落盘仍是物理令牌**（`TryParseDeclaration` 只用于声明解析，`Keymap.json` 存储走 `TryParse`、不认 `mod`，见 §1.2）。
- **不静默丢弃、也不夺键**：声明的默认手势**原样采用**（不再"空槽才落"）。若撞了同作用域的内建 / 别的脚本，
  ...
  用户 override 恒胜声明默认（§7 语义）。之所以不"撞了就丢"：同域冲突还会经手改 `Keymap.json` 等路径产生、
...
持久检测展示**——因为预防覆盖不全（手改 `Keymap.json`、多脚本各自声明同一默认手势都会绕过交互绑定形成同域冲突），
