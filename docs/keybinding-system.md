...
| `transport.gotoStart` | Home | Editor | OnKeyDown（与 FunctionBar 按钮同路径） |
| `transport.gotoEnd` | End | Editor | OnKeyDown（与 FunctionBar 按钮同路径） |
...
- **绑定携带 args**：让一命令参数化服务多绑定（届时工具/移调可回收为「一命令 × 多 args」）。
  （**动作那一侧已经有参数了**：带选择器参数的动作与 `run_action --argument`，见 command-surface.md §5.6。
  缺的仍是**绑定**这一侧——一条绑定只有手势、没有参数，故带**必填**参数的动作今天不可绑：`Keymap.Register`
  当场断言拒绝。带**可选**修饰符参数的动作**可绑**（四条移调即是，见 command-surface.md §5.7）：手势路径
  不带参数、走缺省行为，故它们不需要等 v2；v2 要解决的是"同一条动作绑两个手势、各带不同参数"。）
