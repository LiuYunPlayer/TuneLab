...
> 借鉴业界"脚本文件即工具"的元数据模型，但收敛进 TuneLab 现有的脚本执行底座（`TuneLab.Scripting`）。
...
- 沿用业界脚本工具已通行的形状，用户与 LLM 已熟悉，知识可迁移。
- 本地化等动态元数据天然支持（见 §4）。
...
function getScriptInfo() {
  return {
    ...
    author:   'someone',
    version:  '1.0.0',
    context:  'note',         // note|partContent|pianoSelection|part|trackContent|trackSelection|track|global —— 决定挂哪个菜单 + 快捷键作用区（见 §3）
    id:       'octaveUp',     // 可选：稳定快捷键锚点，独立于文件名、发布后不改（重命名/重装不丢绑定）；省略则用文件名
    defaultGesture: 'mod+up', // 可选：建议快捷键，mod=Mac ⌘/Win Ctrl；仅当该键在本区空闲才生效、绝不覆盖内建
  };
}
...
字段沿用业界通行形状：`name`/`category`/`author`/`version` 是常见约定；`context` 是 TuneLab 扩展。
`id` / `defaultGesture` 服务快捷键系统（见 §3 与 [keybinding-system.md §6](keybinding-system.md)）：`id` 是重命名/重装
仍不丢绑定的稳定锚点（字符集 `[A-Za-z0-9._-]`），`defaultGesture` 是"空槽才落、不夺内建"的建议手势
（`mod`/`primary` 别名 → 本平台主命令键）。均可选，省略即无。
...
各菜单按"右键命中对象 / 命中空白 / 命中范围选区"分支，对应不同的目标心智，故 context 也分开。脚本经"该 context 天经地义的入口"取目标——命中 note/part/track 时该对象必被选中（右键即选中），故一律用选区：

| context | 挂载点 | 目标对象 | 脚本取目标 | 快捷键作用区（scope） |
|---|---|---|---|---|
| `global` | 顶部 **Scripts** 菜单（按 `category` 分组） | 无具体目标 | currentPart / 全工程 | Editor（编辑器内哪都可达） |
| `note` | 钢琴卷帘**命中音符**右键 | 选中的音符 | `tl.currentPart().selectedNotes()` | PianoWindow |
| `partContent` | 钢琴卷帘**空白**右键（part 的内容区） | 当前编辑的 part | `tl.currentPart()` | PianoWindow |
| `pianoSelection` | 钢琴卷帘**命中范围选区**右键（tick 带） | 该时间段 | `tl.pianoSelection()`（无则 null） | PianoWindow |
| `part` | 编排区**命中 part**右键（part 整体对象） | 选中的 parts（可多选） | `tl.selectedParts()` | TrackWindow |
| `trackContent` | 编排区**空白泳道**右键（轨道容器区） | 该泳道的轨道 | `tl.selectedTracks()` | TrackWindow |
| `trackSelection` | 编排区**命中范围选区**右键（tick×轨道） | 该区域 | `tl.trackSelection()`（无则 null） | TrackWindow |
| `track` | **轨道头**右键（整轨对象） | 选中的轨道（可多选） | `tl.selectedTracks()` | TrackWindow |

注入点：`PianoScrollViewOperation`（note 命中音符分支、partContent 空白分支、pianoSelection 命中选区带分支）、`TrackScrollViewOperation`（part 命中 part 分支、trackContent 空白泳道分支、trackSelection 命中选区分支）、`TrackHead`（track，菜单只建一次 → `menu.Opening` 时 `RefreshContextTools` 重建）。

右键即选中：轨道头右键选中本轨（`ITrack : ISelectable`，与 part/note 一致）；编排空白泳道右键选中该行轨道。故 track/trackContent 都用 `tl.selectedTracks()`。命名取 content 对称（`partContent`/`trackContent`）、选区对齐访问器（`pianoSelection`/`trackSelection`）。

**context 与快捷键 scope 是两条正交轴**：context 决定"菜单挂哪 + 作用于哪个目标"；`scope`（末列，由 context 派生）决定"哪个焦点子树分发其快捷键"——见 [keybinding-system.md §6](keybinding-system.md)。**由快捷键触发时没有右键命中点**，故目标一律取当前选区（表中"脚本取目标"本就是选区口径），空选区时 `main()` 原子空转即可。
...
- 不引入独立的 `getTranslations(langCode)` 协议——我们枚举时本就会 eval 顶层、`tl` 始终注入，所以 `getScriptInfo` 里直接读 `tl.language` 即可产出本地化显示名。
...
> 补充（已落地）：命令栈之外还有一类写——**非撤销的设置项**（导出配置：数据层普通属性、写它不产生命令，
> `DiscardTo` 管不着）。`ScriptContext` 对它们做**写前留底 + 回退还原**，故"要么生效、要么没发生"对这批字段
> 同样成立（preview 亦然）；而成功提交后它们**不**随 `Ctrl+Z` 回退——设置项刻意不入撤销栈，与在导出侧栏里改一致。
...
Phase 1 **只做元数据自动注册（纯菜单）**；覆盖表随快捷键系统一并落地——脚本命令的用户重绑 / 冲突消解现由
统一键位系统承载（`getScriptInfo` 增 `id` / `defaultGesture` 声明），见 [keybinding-system.md §6](keybinding-system.md)。
...
> **已落地**：单一命令注册表 + 分发 + 设置页 + 脚本命令（`id` / `defaultGesture` / context→scope 焦点子树）均已实现，
> 权威设计见 [keybinding-system.md](keybinding-system.md)。下列为当初立项时的纲要，保留作背景。
...
> 独立设计见 [script-inputs-and-action-surface.md](script-inputs-and-action-surface.md)。要点：脚本加**可选**
> `getInputConfig(ctx)`（响应式入参 schema，随值重算、可读工程上下文、config 构造 API 同构于冻结的 SDK config 类），运行前弹表单窗；
> `main` 改带参 `main(inputs)`；`run_script`/命名脚本/快捷键/agent 四出口收敛到单一 `ScriptInvocation` 管线；
> agent 写操作装全局分级授权闸门（只读建议/需确认/全自动），骑在现有原子收口上。
