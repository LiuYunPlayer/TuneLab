...
public interface ITuneLabContext
{
    ...
    ILogger GetLogger(string subName);// 在自动前缀后再加子标签
}
...
已落地：
- `SDK.Base/Environment/`：`ITuneLabContext`（`Language` + `GetLogger()`/`GetLogger(subName)`）、静态点 `TuneLabContext.Global`、`NullContext`（`ILogger` 留 `SDK.Base` 根）。
- 宿主：`TuneLabContextGlobal`（`Language` 取 `TranslationManager.CurrentLanguage`；`GetLogger` 按调用者 ALC 自动前缀、转发进 Foundation `Log`）；`Program` 启动期注入 `TuneLabContext.Global`。**ILogger 随之接通。**
