...
| B 包元数据（name / description） | 宿主直接读 `manifest.json`（早于任何插件代码） | **宿主侧**：manifest.json 内 `localizations` 字段，宿主按语言挑、回退基础值 |
...
manifest 是宿主在任何插件代码之前直接读的数据，无法靠插件自译，用 manifest.json 内本地化字段解决：
...
宿主加载按当前语言挑 `localizations[lang]`，缺则回退基础 `name`/`description`（`ExtensionManifest.LocalizedName/LocalizedDescription`）。纯数据、自包含、不依赖宿主 i18n 内部。
...
- 宿主：`ExtensionManifest.localizations` + `LocalizedName/LocalizedDescription`；`ExtensionManager` 加载按当前语言挑 manifest，流入 sidebar。

- 测试夹具：独立 `tests/plugins/V1.I18N`（voice 引擎，插件侧自译演示 + manifest.json `localizations`），用例见 `tests/PLUGIN-I18N-TEST-CASES.md`。
