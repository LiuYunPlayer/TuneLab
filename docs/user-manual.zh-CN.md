...
- 菜单 **文件 → 添加轨道**，或在片段区空白处右键 → **导入音频轨道 / 导入工程轨道**。导入工程轨道时，轨道选择窗里的轨道**默认全选**，不要的取消勾选即可；导入进来的轨道若原本不带颜色（mid 等外部格式普遍不带），会接着当前工程按调色板依次取色。
- 轨道头右键：**导出音频**（单独导这一轨）、**上移**、**下移**、**设置颜色**、**作为参考**（把该轨作为参考显示/隐藏在钢琴窗背景里）、**删除**。
- 轨道可多选：`Ctrl` + 单击加选 / 减选，拖动可整批调位。**删除**（右键菜单或 `Delete` 键）作用于**全部选中的轨道**，一次撤销即可整批还原。
