...
- **坐标一律绝对 tick。** 所有位置/时长都是绝对（全局）tick（`tl.ppq` 取每四分音符的 tick 数，是常量 480）——与播放线、小节同一坐标系。你**永不做坐标换算**。
...
| 成员 | 返回 | 说明 |
|---|---|---|
| `tl.ppq` | number | 每**四分音符**的 tick 数——常量 480，不是工程的属性。而一**拍**未必是四分音符：拍长由拍号分母定（4/4 → `ppq`，6/8 → `ppq/2`），故要从 `project.timeSignatures()` 读拍号，别拍脑袋。 |
...
| 字段 | 读写 | 含义 |
|---|---|---|
| `pos` | 读写 | **锚点**的绝对 tick。它同时是 part 在**内部**存内容所用的坐标原点——这正是给 `pos` 赋值会**平移整段**（内容跟随、长度不变）的原因。但这套换算你一次都不用自己做，见表下那条。 |
| `startOffset` | 读写 | **左边缘**相对锚点的有符号偏移：`>0` 前向裁剪，`<0` 前向扩展。裁剪只动边缘，内容留在原处。 |
| `endOffset` | 读写 | **右边缘**相对锚点的有符号偏移（拖右边缘就是改它）。 |
| `startPos` | 只读 | `= pos + startOffset` —— part 的**第一个 tick**。「part 的开头」指的就是它：放在最开头的音符写作 `{pos: part.startPos, …}`。 |
| `endPos` | 只读 | `= pos + endOffset` —— part 最后一个 tick 的**下一个**（右开区间的端点）。 |
| `dur` | 只读 | `= endPos - startPos = endOffset - startOffset` |

落在 `[startPos, endPos)` 之外的内容**仍然存在**，只是被**裁掉**（不发声、不显示）——两个偏移量就是干这个用的。

> **动作面上没有任何相对 tick。** 你在音符 / 颤音 / 曲线点上读到或写入的每一个 tick 都是**绝对**的（全局时间线），已经含了 part 的锚点。既不要给它加 `part.pos`，也不要减。`pos` 是存储原点这件事纯属内部事实，对外只体现为「平移 part 时内容跟着走」。
