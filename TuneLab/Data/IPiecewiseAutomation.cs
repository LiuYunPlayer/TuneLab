...
internal static class IPiecewiseAutomationExtension
{
    ...
    public static Map<string, PiecewiseAutomationInfo> PiecewiseAutomationsToInfo(this IReadOnlyDataObjectMap<string, IPiecewiseAutomation> map)
    {
        var info = new Map<string, PiecewiseAutomationInfo>();
        foreach (var kvp in map)
        {
            info.Add(kvp.Key, new PiecewiseAutomationInfo { Segments = kvp.Value.GetInfo() });
        }
        return info;
    }

    public static Map<string, IPiecewiseAutomation> ToPiecewiseAutomations(this IReadOnlyMap<string, PiecewiseAutomationInfo> info)
    {
        var map = new Map<string, IPiecewiseAutomation>();
        foreach (var kvp in info)
        {
            var automation = new PiecewiseAutomation();
            automation.SetInfo(kvp.Value.Segments);
