...
// 两个载体同族、共用本文件的换算 / 裁剪 / 简化：合成参数 → 同 Id 可编辑轨、合成音高 → Pitch。
//
...
//
// 刻意只做显式一次性动作、不做持续同步：自动跟随会形成 用户覆盖 → 重合成 → 合成参数变 → 再写入 的反馈环，
// 且把合成结果混进 undo 栈（撤销栈里出现用户没做过的变更）。
//
// 值按真实数值原样写入、不按目标轨量程钳位——钳位会静默改数据；量程只定义值轴形状（见 AutomationConfig.Scale），
// 越界如实。合成参数轨与编辑轨各按自己的量程绘制、宿主不要求二者一致，声明不一致的后果就是显示错位与固定值出界。
//
...
internal static class SynthesisLock
{
    ...
    // 该 key 是否配对（同 Id 既声明了可编辑轨、又声明了合成参数轨）。固定笔刷据此决定是否进入操作态。
    public static bool HasPairedSynthesizedParameter(this IMidiPart part, AutomationKey key)
    {
        if (key.IsLane)
            return false;
        ...
        return GetSynthesizedParameterDeclarations(part, key)?.ContainsKey(key.Id) ?? false;
    }

    // 冻结当前合成参数产物引用，供笔刷式操作在 Down 时取一次、整笔复用。
    // **必须冻结**：笔刷第一帧写入就会触发合成失效、引擎可能随即清掉该区产物，后续帧再读实时产物就是空的，
    // 整笔会什么都写不进去。产物按 SDK 契约「发布即不可变、变化时换引用」，故持旧引用安全。
    //（区间失效本身已纳入批量括号、拖动期不转发，见 VoiceSynthesisContext.FlushPendingRanges；本冻结是第二重防御。）
    public static IReadOnlyList<IReadOnlyList<Point>>? CaptureSynthesizedParameter(this IMidiPart part, AutomationKey key)
    {
        return GetSynthesizedParameterSegments(part, key);
    }

    // 裸写版（不含事务）：供笔刷式操作复用——拖动期每帧 DiscardTo(head) 后按新范围重写，抬手才 Commit。
    // 调用方负责 BeginMergeDirty / EndMergeDirty / Commit，并传入 Down 时冻结的 segments（见 CaptureSynthesizedParameter）。
    //
    // 返回【是否真的写进了东西】：区间内没有产物（未合成 / 该段产物为空）时是 no-op 并返回 false。笔刷式调用
    // 忽略它即可（用户看得见结果）；脚本面把它当返回值回报——那边没人盯着屏幕，静默 no-op 会被当成固定成功。
    public static bool WriteSynthesizedParameterLock(this IMidiPart part, AutomationKey key, IReadOnlyList<IReadOnlyList<Point>>? segments, double startTick, double endTick,
                                        bool subtractVibrato = true)
    {
        ...
        if (part.IsEffectivePiecewiseAutomation(key))
        {
            ...
            // 「曲线 + 偏移」、合成参数含偏移；而原本 NaN 处引擎用的是自己的模型值、合成参数**不含**偏移，
            // 若一并扣就会把曲线整体压低一个颤音幅度。原曲线要在写入前取（写完就有值了）。
            //
            // subtractVibrato=false 用于**空隙填充**（LockSynthesizedParameterGaps）：那一整段在固定前都是 NaN，
            // 合成参数整段不含用户侧偏差，一律不扣。不能靠逐点查 existing 判——空隙的边界点坐标恰好等于相邻
            ...
            if (part.AddEffectivePiecewiseAutomation(key) is not { } target)
                return false;
        ...
        else if (part.IsEffectiveAutomation(key))
        {
            ...
            if (part.AddEffectiveAutomation(key) is not { } target)
                return false;
    ...
    // 用途：颤音只作用于有值段（NaN 段无基线可叠），故用户把颤音关联到一条有配对合成参数的分段轨时，
    ...
    public static void LockSynthesizedParameterGaps(this IMidiPart part, AutomationKey key, double startTick, double endTick)
    {
        if (!part.IsEffectivePiecewiseAutomation(key) || !part.HasPairedSynthesizedParameter(key))
            return;

        var segments = GetSynthesizedParameterSegments(part, key);
        ...
        // 整段不扣 vibrato：空隙在固定前是 NaN，引擎在那儿用的是自己的模型值、合成参数不含用户侧偏差。
        foreach (var gap in gaps)
            part.WriteSynthesizedParameterLock(key, segments, gap.Start + partPos, gap.End + partPos, subtractVibrato: false);
    }
    ...
    public static void LockAssociatedSynthesizedParameterGaps(this IMidiPart part, Vibrato vibrato)
    {
        ...
        {
            if (kvp.Value != 0)
                part.LockSynthesizedParameterGaps(AutomationKey.Voice(kvp.Key), startTick, endTick);
        }
        ...
        for (int i = 0; i < part.Effects.Count; i++)
        {
            ...
            foreach (var kvp in vibrato.AffectedEffectAutomations)
            {
                if (kvp.Value != 0 && kvp.Key.EffectId == effectId)
                    part.LockSynthesizedParameterGaps(AutomationKey.Effect(i, kvp.Key.Id), startTick, endTick);
            }
        }
    }

    // —— 合成音高 → Pitch（专属常驻通道，与合成参数同族）——
    // SynthesizedPitch 与 Pitch 同值域（半音、同一口径，绘制端两者同经 Pitch2Y(value + 0.5)），故直传。

    // 冻结当前合成音高产物引用（同 CaptureSynthesizedParameter 的理由：笔刷第一帧写入即触发失效、引擎可能清掉产物）。
    public static IReadOnlyList<IReadOnlyList<Point>> CaptureSynthesizedPitch(this IMidiPart part)
    {
        return part.SynthesizedPitch;
    }

    // 裸写版（不含事务），供固定笔刷复用；与 WriteSynthesizedParameterLock 对偶（返回值口径同它）。
    // 合成音高已含 vibrato（引擎收的是 Pitch + PitchDeviation 两条、把偏差加了进去），故写进 Pitch 前须扣掉它。
    public static bool WriteSynthesizedPitchLock(this IMidiPart part, IReadOnlyList<IReadOnlyList<Point>> segments, double startTick, double endTick)
    {
    ...
    static IReadOnlyOrderedMap<PropertyKey, AutomationConfig>? GetSynthesizedParameterDeclarations(IMidiPart part, AutomationKey key)
    {
        if (key.IsEffect)
    ...
    static IReadOnlyList<IReadOnlyList<Point>>? GetSynthesizedParameterSegments(IMidiPart part, AutomationKey key)
    {
        if (key.IsLane)
            return null;
    ...
    // 段间间隙（合成参数的 NaN 区）不产生线，故分段轨固定后仍在那些位置留空、连续轨则保留原值。
    //
                                          ...
                                          Func<IReadOnlyList<double>, double[]>? deviation = null)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            foreach (var point in segment)
            {
                ...
                // 同 tick 重复点（产物帧率高于 tick 分辨率处）会让插值退化，取先到的那个。
