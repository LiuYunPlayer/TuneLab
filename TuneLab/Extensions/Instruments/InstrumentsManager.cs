...
internal static class InstrumentsManager
{
    ...
    {
        foreach (var type in mInstrumentEngines.Keys)
        {
            foreach (var engine in mInstrumentEngines[type])
            {
                if (!engine.IsInited)
                    continue;
                Log.Info(string.Format("Destroying instrument engine [{0}] from package [{1}]...", type, engine.PackageId));
                try { engine.Engine.Destroy(); }
                catch (Exception ex) { Log.ErrorAttributed(string.Format("Instrument engine [{0}] failed to destroy", type), ex); }
            }
        }
    }
