...
internal sealed class RunScriptTool(IProject project, Func<IMidiPart?>? currentPart, Func<IQuantization?>? quantization, Func<string?>? language, Func<ScriptSelection?>? selection, Func<ScriptPianoSelection?>? pianoSelection) : IAgentTool
{
    ...
    public async Task<string> ExecuteAsync(string argumentsJson, CancellationToken cancellationToken)
    {
        ...
        try
        {
            result = await Dispatcher.UIThread.InvokeAsync(() => ScriptRunner.Run(project, currentPart, quantization, language, selection, pianoSelection, ScriptLimits.Agent, code, cancellationToken));
            int waited = 0;
