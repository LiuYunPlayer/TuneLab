...
internal sealed class ScriptWriteExecutor(IProject project, Func<IMidiPart?>? currentPart, Func<IQuantization?>? quantization, Func<string?>? language, Func<ScriptSelection?>? selection, Func<ScriptPianoSelection?>? pianoSelection, Func<AuthorizationRequest, CancellationToken, Task<ScriptAuthDecision>>? confirm = null)
{
    ...
    public async Task<string> RunWithAuthorizationAsync(string code, PropertyObject? inputs, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var decision = await confirm(new AuthorizationRequest(WriteKind.ProjectEdit, pv.Changes, null), cancellationToken);
