...
internal sealed class ActionParameter
{
    ...
    // true = 这个参数**可选**：不给值时动作照样跑，走 EditorAction.Execute 那条缺省路径。
    //
    // 【它不违反上面那条"成员固定就逐值开动作"的判据】必填的那种参数选的是**作用对象**（哪条参数轨、
    // 哪个属性），一个动词配一个成员，加成员就是加值。可选的这种是**修饰符**：同一件事做不做连带的
    // 副作用（移调要不要把音高线与自动化曲线一起搬走）。修饰符逐值开动作会把动作**成倍**翻（四条移调
    // → 八条），而且这批动作【带手势】——键盘按下去时无从携带参数，只能有一条缺省行为。故形状是
    //「一条动作 + 一个可选修饰符」：手势与界面入口走缺省，命令面要确定性行为就显式给值。
    //
    // 与必填参数的另一处不同：**带可选参数的动作可以绑手势**（Keymap.Register 只拒必填的），手势路径
    // 不带参数、走 Execute。
    public bool Optional { get; init; }

    // 仅可选参数：不给值时会发生什么（一句英文，原样进 `action list` 的回报）。**必填**——缺省行为若
    // 依赖用户的某个设置，这里必须点名是哪个设置，否则调用方根本不知道自己拿到的是哪种行为，
    // 而"想要确定性就显式给值"这句建议也就无从落地。
